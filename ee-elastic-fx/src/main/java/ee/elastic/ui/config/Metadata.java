...
public class Metadata {
	...
	public Metadata(ClusterState state, Map<String, IndexStatus> status,
			Map<String, IndexStats> stats) {
		super();
	...
	public boolean isLoadParents(String index, String type) {
		...
		return config != null && config.isLoadParents() != null
				&& config.isLoadParents();
	}
	...
	public void init(ClusterState state, Map<String, IndexStatus> status,
			Map<String, IndexStats> stats) {
		this.state = state;
		this.status = status;
		this.stats = stats;
		...
		for (ObjectObjectCursor<String, IndexMetaData> indexMetaDataCursor : state
				.getMetaData().getIndices()) {
			IndexMetaData indexMetaData = indexMetaDataCursor.value;
			for (ObjectObjectCursor<String, MappingMetaData> mappingMetaDataCursor : indexMetaData
					.getMappings()) {
				MappingMetaData mappingMetaData = mappingMetaDataCursor.value;
				indexTypes
						.add(indexMetaData.getIndex(), mappingMetaData.type());
				try {
					Map<String, Object> sourceAsMap = mappingMetaData
							.getSourceAsMap();
					for (Map.Entry<String, Map<String, ?>> field : ((Map<String, Map<String, ?>>) sourceAsMap
							.get("properties")).entrySet()) {
						Field typeField = new Field(field.getKey(),
								String.valueOf(field.getValue().get("type")));
						indexFields.add(indexMetaData.getIndex(), typeField);
						typeFields.put(indexMetaData.getIndex(),
								mappingMetaData.type(), field.getKey(),
								typeField);
					}
					typeMappings.put(indexMetaData.index(), mappingMetaData
							.type(), new TypeMapping(mappingMetaData.type(),
							sourceAsMap));
					...
					indexFields.add(indexMetaData.getIndex(), typeField);
					typeFields.put(indexMetaData.getIndex(),
							mappingMetaData.type(), "_all", typeField);
	...
	public Set<Field> fields(Set<String> indices, Set<String> types) {
		...
		for (String index : indices.isEmpty() ? typeFields.parentKeys()
				: indices) {
			for (String type : types.isEmpty() ? typeFields.childKeys(index)
					: types) {
