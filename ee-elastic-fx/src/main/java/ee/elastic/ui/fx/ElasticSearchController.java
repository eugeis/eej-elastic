	...
	private final class TableSelectionListener implements ChangeListener {
		...
		public void changed(ObservableValue observable, Object oldValue, Object newValue) {
			if (newValue != null && newValue instanceof Map) {
				...
				ViewDef viewDef = model.metadata().view(String.valueOf(row.get("_index")),
						String.valueOf(row.get("_type")));
		...
		if (url != null) {
			try {
				...
				if (model.isConnected()) {
					...
					overviewController = new OverviewController(stage, model, indicesTable, detailsTypes,
							detailsTypeMapping, detailsStatus, detailsMetadata);
					fillAllIndices();
				}
