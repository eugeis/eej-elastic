...
public class SearchController {
	...
	WeakInvalidationListener weakColumnSortableObserver = new WeakInvalidationListener(
			columnSortableObserver);
	WeakInvalidationListener weakColumnSortTypeObserver = new WeakInvalidationListener(
			columnSortTypeObserver);

	private final class CellFactoryBuilder implements
			Func<ColumnDef, Callback<TableColumn, TableCell>> {
		private GenericCellFactory call;

		public CellFactoryBuilder(final TableView tableView,
				final TableModelByColumns tableModel) {
			super();
	...
	private final class TableColumnBuilder implements
			Func<ColumnDef, TableColumn> {
		CellFactoryBuilder cellFactoryBuilder;

		public TableColumnBuilder(TableView tableView,
				TableModelByColumns tableModel) {
			super();
			this.cellFactoryBuilder = new CellFactoryBuilder(tableView,
					tableModel);
		}
		...
		public TableColumn call(ColumnDef columnDef) {
			...
			column.setCellValueFactory(new ColumnValueFactory(columnDef,
					converters.converter(columnDef), columnsValuesCache));
			column.setCellFactory(cellFactoryBuilder.call(columnDef));
			return column;
		}
	}

	private final class SortOrderColumnBuilder implements
			Func<TableColumn, Sort<String, SortType>> {
		@Override
	...
	private final class ColumnTextSelector implements
			Func<TableColumn, Boolean> {
		private String text;
	...
	public SearchController(Model model, TableView resultTable,
			Label searchStatus, ChoiceBox<Integer> rowCount) {
		super();
	...
	private void addSortOrderListener() {
		resultTable.getSortOrder().addListener(
				new ListChangeListener<TableColumn>() {
					@Override
					public void onChanged(
							javafx.collections.ListChangeListener.Change<? extends TableColumn> change) {
						updateSortOrder();
					}
				});
	}
	...
	private void addRowCountListener(ChoiceBox<Integer> rowCountControl) {
		...
		rowCountControl.getSelectionModel().selectedItemProperty()
...
//				}));
	...
	protected void doSearch() {
		...
		final ObservableList<Map<String, ?>> data = FXCollections
				.observableArrayList(tableModel.getItems());
		...
		Platform.runLater(new Runnable() {
			...
			private void updateSearchStatusLabel(final SearchResponse response) {
				String status = String.format(
						"Searched %s of %s shards, %s hits, %s seconds.",
						response.getSuccessfulShards(), response
								.getTotalShards(), response.getHits()
								.getTotalHits(), response.getTook()
								.getSecondsFrac());
			...
			private void updateTable(final ObservableList<Map<String, ?>> data) {
				...
				if (sorting) {
					sortOrder = Utils.transform(resultTable.getSortOrder(),
							sortOrderColumnBuilder);
				}
				resultTable.getItems().clear();
				if (tableModel.isColumnChanged()) {
					resultTable.getColumns().clear();
					resultTable.getColumns().addAll(
							Utils.transform(tableModel.getColumns(),
									tableColumnBuilder));
				}
				resultTable.setItems(data);
				if (sorting && sortOrder != null) {
					for (Sort<String, SortType> sort : sortOrder) {
						TableColumn column = Utils.findFirst(
								resultTable.getColumns(),
								new ColumnTextSelector(sort.element()));
						if (column != null) {
